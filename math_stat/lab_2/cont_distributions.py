...
def create_interval_error_table(emp, theory):
    struct = {"Intervals":emp.df["Intervals"], "Frequency":emp.df["Frequency"]}
    ...
    print("sum : ", sum(table["theory_prob"]), end='\n')
    return table


class Empirical(Base):
    def __init__(self, data, type_="Unknown"):
        super().__init__()
        self.type = type_
        self.__size = len(data)
        self.__data = np.sort(data)
        self.__create_bins()
        self.df = self.__create_interval_df()  # main interval dataframe
        self.__add_midpoint()
        # counting:
        self.__count_mean()
    ...
    def __count_centre_k_moment(self, k):
        return sum((self.df.Midpoint - self.mean) ** k * self.df.Frequency)

    def __count_k_moment(self, k):
        return sum((self.df.Midpoint ** k) * self.df.Frequency)
    ...
    def __count_median(self):
        ...
        s = self.df["Frequency"][0]
        k = 0
        while s <= 1 / 2:
            k += 1
            s += self.df["Frequency"][k]
        w_k = self.df["Frequency"][k]
        left = self.df["Intervals"][k].left
        right = self.df["Intervals"][k].right
        if s == 1 / 2:
            self.median = right
        else:
    ...
    def __count_mode(self):
        idx_max = self.df["Count"].idxmax()
        max_freq = self.df["Frequency"][idx_max]
        right_freq, left_freq = 0, 0
        if idx_max > 0:
            left_freq = self.df["Frequency"][idx_max - 1]
        if idx_max + 1 < prm.count_intervals:
            right_freq = self.df["Frequency"][idx_max + 1]
        h = (self.__bins[-1] - self.__bins[0]) / prm.count_intervals
        left_mode = self.df["Intervals"][idx_max].left
