...
#define MAX_PRINTABLE_NUMBER 4

int read_matrix(const char *name, double *matrix, int n)
{
	FILE *fp;
	int idx1, idx2;
	...
	for (idx1 = 0; idx1 < n; idx1++)
	{
		...
		{
			if (fscanf(fp, "%lf", matrix + idx2 * n + idx1) != 1)
			{
				if (!feof(fp))
				{
					fclose(fp);
...
void init_matrix(double *matrix, int n)
{
	...
	for (idx1 = 0; idx1 < n; idx1++)
	{
		for (idx2 = 0; idx2 < n; idx2++)
		{
			matrix[idx2 * n + idx1] = init_element(idx1, idx2);
		}
	}
	return;
}
...
{
	return fabs(idx1-idx2);//1.0 / (idx1 + idx2 + 1);
}
...
void print_matrix(double *matrix, int n, int m)
{
	...
	for (idx1 = 0; idx1 < left_board1; idx1++)
	{
		...
		for (idx2 = 0; idx2 < left_board2; idx2++)
		{
			printf(" %15.4f", matrix[idx1 * m + idx2]);
		}
		if (2 * MAX_PRINTABLE_NUMBER < m)
		{
			printf("  ...  ");
