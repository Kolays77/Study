...
void student_list::delete_next () //+
{
   ...
   next_curr->set_previous (curr);
}

void student_list::delete_curr ()
{
   student_node *new_curr = curr->get_next(), *prev = curr->get_previous();

   if (curr == tail)
     {
       student_list::delete_last();
       curr = prev;
       return;
     }
   if (curr == head)
     {
       student_list::delete_first();
       curr = prev;
       return;
     }
   delete curr;
   curr = prev;
   prev->set_next (new_curr);
   new_curr->set_previous (prev);
   return;
}
