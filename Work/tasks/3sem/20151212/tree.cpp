...
void make_first (tree_node *root)
{
  if (!root || !root->get_child())
  {
      return;
  }

  tree_node *first_child = root->get_child();
  for (tree_node *cur = first_child; cur; cur =  cur->get_next())
  {
      cur->set_first (first_child);
      make_first (cur);
  }
}
